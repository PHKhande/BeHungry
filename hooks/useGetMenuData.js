import { SWIGGY_RESTAURANT_MENU_LINK, RESTAURANT_TYPE_KEY, MENU_ITEM_TYPE_KEY } from "../src/Constants";

import { useEffect, useState } from "react";

const useGetMenuData = (restaurantId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        menuItemsData.forEach((item) => {
          if (!uniqueMenuItems.find((x) => x.id === item.id) && uniqueMenuItems.length<30) {
